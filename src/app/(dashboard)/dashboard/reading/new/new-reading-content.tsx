...
export function NewReadingContent() {
  ...
  const handleResumeReading = async () => {
    const tempToken = sessionStorage.getItem("tempReadingToken");
    if (!tempToken) {
      setError("No reading session found. Please upload a new photo.");
      return;
    }
    ...
    try {
      ...
      // Get temp reading data
      const tempData = JSON.parse(atob(tempToken));
      ...
      // Clean up temp token
      sessionStorage.removeItem("tempReadingToken");
